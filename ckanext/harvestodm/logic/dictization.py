...
from ckanext.harvestodm.model import (HarvestSource, HarvestJob,
                                      HarvestObject, HarvestGatherError,
                                      HarvestObjectError)
...
def harvest_job_dictize(job, context):
    ...
    if context.get('return_error_summary', True):
        q = model.Session.query(
            ...
            func.count(HarvestObjectError.message)
            .label('error_count'))\
            .join(HarvestObject)\
            .filter(HarvestObject.harvest_job_id == job.id)\
        ...
        q = model.Session.query(
            ...
            func.count(HarvestGatherError.message)
            .label('error_count'))\
            .filter(HarvestGatherError.harvest_job_id == job.id)\
...
def _get_source_status(source, context):
    ...
    if last_job:
        ...
        if detailed:
            ...
            statistics['errors'] = (harvest_job_dict['stats'].get(
                'errored', 0) + len(last_job.gather_errors))

        if detailed:
            # We have the gathering errors in last_job.gather_errors,
            # so let's also get also the object errors.
            object_errors = model.Session.query(HarvestObjectError).join(
                HarvestObject) .filter(HarvestObject.job == last_job)
            for gather_error in last_job.gather_errors:
                out['last_harvest_errors'][
                    'gather'].append(gather_error.message)

            for object_error in object_errors:
                err = {
                    'object_id': object_error.object.id,
        ...
        packages = model.Session.query(
            ...
            Package.name) .join(Package).join(HarvestSource) .filter(
            HarvestObject.source == source) .filter(
                HarvestObject.current) .filter(
