...
from ckanext.harvestodm.model import (HarvestSource, HarvestJob, HarvestObject,
                                      HarvestObjectExtra)
from ckanext.harvestodm.logic.dictization import (harvest_job_dictize,
                                                  harvest_object_dictize)
from ckanext.harvestodm.logic.schema import (
    harvest_source_show_package_schema,
    harvest_object_create_schema)
from ckanext.harvestodm.logic.action.get import (harvest_source_list,
                                                 harvest_job_list)
...
def harvest_job_create_all(context, data_dict):
    ...
    for source in sources:
        ...
        if exists:
            log.info(
                'Skipping source %s as it already has a pending job',
                source['id'])
            continue
