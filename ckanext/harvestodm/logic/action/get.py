...
from ckanext.harvestodm import model as harvest_model

from ckanext.harvestodm.model import (HarvestSource, HarvestJob, HarvestObject)
from ckanext.harvestodm.logic.dictization import (harvest_source_dictize,
                                                  harvest_job_dictize,
...
def harvest_object_list(context, data_dict):
    ...
    if only_current:
        query = query.filter(HarvestObject.current)
...
def harvesters_info_show(context, data_dict):
    ...
    for harvester in PluginImplementations(IHarvester):
        ...
        if not info or 'name' not in info:
            log.error(
                'Harvester %r does not provide the harvester name '
                'in the info response' % str(harvester))
            continue
