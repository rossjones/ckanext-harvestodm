import ckanext.harvestodm.model as harvest_model
from ckanext.harvestodm.model import HarvestObject, HarvestObjectExtra
from ckanext.harvestodm.interfaces import IHarvester
import ckanext.harvestodm.queue as queue
...
class TestHarvestQueue(object):
    ...
    def test_01_basic_harvester(self):
        ...
        count = model.Session.query(model.Package) \
            ...
            .count()
        assert count == 3
        all_objects = model.Session.query(
            HarvestObject).filter_by(current=True).all()
        ...
        assert harvest_source_dict['status'][
            'last_job']['stats'] == {'added': 3}
        ...
        all_objects = model.Session.query(
            HarvestObject).filter_by(report_status='added').all()
        assert len(all_objects) == 3, len(all_objects)

        all_objects = model.Session.query(
            HarvestObject).filter_by(report_status='updated').all()
        assert len(all_objects) == 2, len(all_objects)

        all_objects = model.Session.query(
            HarvestObject).filter_by(report_status='deleted').all()
        ...
        assert harvest_source_dict['status']['last_job'][
            'stats'] == {'updated': 2, 'deleted': 1}
