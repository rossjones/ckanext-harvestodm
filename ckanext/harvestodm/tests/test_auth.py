...
from ckanext.harvestodm.plugin import Harvest
from ckanext.harvestodm.model import HarvestSource, HarvestJob, setup as harvest_model_setup
...
class HarvestAuthBaseCase():
    ...
    def _test_auth_allowed(self, user_name, auth_profile=None):
        ...
        res = self.app.get(
            '/harvest/refresh/%s' %
            source.id, extra_environ=extra_environ)
...
class TestAuthDefaultProfile(FunctionalTestCase, HarvestAuthBaseCase):
    ...
    def setup_class(cls):
        if (config.get('ckan.harvest.auth.profile', '') != ''):
            raise SkipTest(
                'Skipping default auth profile tests. Set ckan.harvest.auth.profile = \'\' to run them')
...
class TestAuthPublisherProfile(FunctionalTestCase, HarvestAuthBaseCase):
    ...
    def setup(self):
        ...
        self.sysadmin_user = model.User.get('testsysadmin')
        # Does not belong to a publisher
        self.normal_user = model.User.get('annafan')
        ...
        self.publisher1 = model.Group(
            name=u'test-publisher1',
            title=u'Test Publihser 1',
            type=u'publisher')
        Session.add(self.publisher1)
        self.publisher2 = model.Group(
            name=u'test-publisher2',
    ...
    def test_auth_publisher_profile_sysadmin(self):
        self._test_auth_allowed(
            self.sysadmin_user.name,
            auth_profile='publisher')

    def test_auth_publisher_profile_publisher(self):
        self._test_auth_allowed(
            self.publisher1_user.name,
            auth_profile='publisher')
    ...
    def test_auth_publisher_profile_different_publisher(self):
        ...
        # Check that this publihser is not allowed to manage sources from other
        # publishers
        status = 401
        # Read
        res = self.app.get(
