...
class HarvestSourceActionBase(object):
    ...
    def test_invalid_unknown_frequency(self):
        ...
        assert u'Frequency {0} not recognised'.format(
            wrong_frequency) in result['frequency'][0]
    ...
    def test_invalid_wrong_configuration(self):
        ...
        assert u'Error parsing the configuration options: No JSON object could be decoded' in result[
            'config'][0]
        ...
        assert 'config' in result
        assert u'Error parsing the configuration options: custom_option must be a list' in result[
            'config'][0]
...
class TestHarvestSourceActionCreate(HarvestSourceActionBase):
    ...
    def test_create(self):
        ...
        assert u'There already is a Harvest Source for this URL' in result[
            'url'][0]
...
class TestHarvestObject(unittest.TestCase):
    ...
    def test_create(self):
        ...
        assert created_object.guid == harvest_object[
            'guid'] == data_dict['guid']
